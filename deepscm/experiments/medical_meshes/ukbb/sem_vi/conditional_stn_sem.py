...
class ConditionalSTNVISEM(BaseVISEM):
    def __init__(self, **kwargs):
        ...
        structure_volume_net = DenseNN(2, [8, 16], param_dims=[1, 1], nonlinearity=torch.nn.Identity())
        self.structure_volume_flow_components = ConditionalAffineTransform(context_nn=structure_volume_net, event_dim=0)
        self.structure_volume_flow_lognorm = AffineTransform(loc=0., scale=1.)
        self.structure_volume_flow_constraint_transforms = ComposeTransform([self.structure_volume_flow_lognorm, ExpTransform()])
        self.structure_volume_flow_transforms = [self.structure_volume_flow_components, self.structure_volume_flow_constraint_transforms]
    ...
    def pgm_model(self):
        ...
        structure_volume_dist = ConditionalTransformedDistribution(structure_volume_base_dist, self.structure_volume_flow_transforms).condition(context)
        ...
        _ = self.structure_volume_flow_transforms
    ...
    def model(self):
        ...
        latent = torch.cat([z, age_, structure_volume_, brain_volume_], 1)
    ...
    def guide(self, x, age, sex, structure_volume, brain_volume):
        with pyro.plate('observations', x.shape[0]):
            ...
            hidden = torch.cat([hidden, age_, structure_volume_, brain_volume_], 1)
