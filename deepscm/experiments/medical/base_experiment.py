...
class BaseCovariateExperiment(pl.LightningModule):
    ...
    def prepare_data(self):
        ...
        split_dir = self.hparams.split_dir \
            if hasattr(self.hparams, 'split_dir') \
            else '/vol/biomedic2/np716/data/gemini/ukbb/ventricle_brain/'
        data_dir = self.hparams.data_dir \
            if hasattr(self.hparams, 'data_dir') \
            else '/vol/biomedic2/bglocker/gemini/UKBB/t0/'
        self.ukbb_train = UKBBDataset(
            f'{split_dir}/train.csv',
            base_path=data_dir,
            crop_type=train_crop_type,
            downsample=downsample
        )  # noqa: E501
        self.ukbb_val = UKBBDataset(
            f'{split_dir}/val.csv',
            base_path=data_dir,
            crop_type='center',
            downsample=downsample
